<?php
    ...
    use App\Models\Billing\Catalogs\AddressType;
     ...
     * @property-read \App\Models\Billing\Person       $parent_person
     * @property-read \App\Models\Billing\Person       $children_person
    ...
    class Person extends ModelTenant
    {
        ...
        {
            if (!Schema::connection('tenant')->hasTable('persons') ||
                !Schema::connection('tenant')->hasColumn('persons', 'enabled')) {
                return $query;
            }
        ...
        public function getCollectionData($withFullAddress = false, $childrens = false, $servers=false)
        {
            ...
            } elseif ($this->relationLoaded('addresses')) {
                $addresses = $this->addresses;
            }
            ...
            $departmentModel = $this->relationLoaded('department') ? $this->department : null;
            if (!$departmentModel && $this->department_id) {
                $departmentModel = Department::find($this->department_id);
            }
            ...
            $provinceModel = $this->relationLoaded('province') ? $this->province : null;
            if (!$provinceModel && $this->province_id) {
                $provinceModel = Province::find($this->province_id);
            }
            $province = $provinceModel ? [
                "id" => $provinceModel->id,
                "description" => $provinceModel->description,
                "active" => $provinceModel->active,
            ] : null;

            $districtModel = $this->relationLoaded('district') ? $this->district : null;
            if (!$districtModel && $this->district_id) {
                $districtModel = District::find($this->district_id);
            }
            $district = $districtModel ? [
                "id" => $districtModel->id,
                "description" => $districtModel->description,
                "active" => $districtModel->active,
            ] : null;

            $sellerModel = $this->relationLoaded('seller') ? $this->seller : null;
            if (!$sellerModel && $this->seller_id) {
                $sellerModel = User::find($this->seller_id);
            }
            $seller = $sellerModel ? $sellerModel->getCollectionData() : null;

            $zone = $this->relationLoaded('zone') ? $this->zone : null;
            if (
                !$zone &&
                $this->zone_id &&
                class_exists(Zone::class) &&
                Schema::connection('tenant')->hasTable('zones')
            ) {
                try {
        ...
        public function scopeWhereFilterCustomerBySeller($query, $type)
        {
            if($type === 'customers')
            {
                ...
                if($user && $user->applyCustomerFilterBySeller())
                {
