...
use App\Models\Billing\Skin;
use Throwable;

    /**
     * Class App\Models\Billing\Configuration
     *
     * @property int         $id
    ...
    class Configuration extends ModelTenant
    {
        ...
        public static function getApiServiceToken()
        {
            ...
            if (!$configuration) return config('configuration.api_service_token');
            
            return $configuration->token_apiruc == 'false' ? config('configuration.api_service_token') : $configuration->token_apiruc;
        }
        ...
        {
            $defaultTemplate = config('tenant.pdf_template', 'default');

            $user = Auth::user();
            if (!empty($user)) {
                $establishment = $user->establishment;
                if (!empty($establishment)) {
                    $template = $establishment->template_pdf;
            ...
            if (!empty($this->formats) && $this->formats !== 'default') {
                return $this->formats;
            }
        ...
        public static function isEnabledLegendForestToXml()
        {
            try {
                if (!Schema::connection('tenant')->hasTable('configurations') ||
                    !Schema::connection('tenant')->hasColumn('configurations', 'legend_forest_to_xml')) {
                    return false;
                }

                $configuration = Configuration::query()->select('legend_forest_to_xml')->first();
                return $configuration ? (bool) $configuration->legend_forest_to_xml : false;
            } catch (Throwable $e) {
                return false;
            }
        }
