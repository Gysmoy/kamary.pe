...
use App\Models\Billing\Catalogs\AffectationIgvType;
use App\Models\Billing\Catalogs\CatColorsItem;
use App\Models\Billing\Catalogs\CatItemMoldCavity;
use App\Models\Billing\Catalogs\CatItemMoldProperty;
use App\Models\Billing\Catalogs\CatItemPackageMeasurement;
use App\Models\Billing\Catalogs\CatItemProductFamily;
use App\Models\Billing\Catalogs\CatItemStatus;
use App\Models\Billing\Catalogs\CatItemUnitBusiness;
use App\Models\Billing\Catalogs\CurrencyType;
use App\Models\Billing\Catalogs\SystemIscType;
use App\Models\Billing\Catalogs\UnitType;
use Carbon\Carbon;
use Exception;
 ...
 * @property \Illuminate\Database\Eloquent\Collection|\App\Models\Billing\DispatchItem[] $dispatch_items
 * @property int|null $dispatch_items_count
 * @property \Illuminate\Database\Eloquent\Collection|\App\Models\Billing\DocumentItem[] $document_items
 * @property int|null $document_items_count
 * @property mixed $attributes
 * @property \Illuminate\Database\Eloquent\Collection|\App\Models\Billing\ItemImage[] $images
 * @property int|null $images_count
 * @property \Illuminate\Database\Eloquent\Collection|\App\Models\Billing\InventoryKardex[] $inventory_kardex
 ...
 * @property \Illuminate\Database\Eloquent\Collection|\App\Models\Billing\ItemMovementRelExtra[] $item_movement_rel_extra
 * @property int|null $item_movement_rel_extra_count
 * @property \App\Models\Billing\ItemType $item_type
 * @property \Illuminate\Database\Eloquent\Collection|\App\Models\Billing\ItemUnitType[] $item_unit_types
 * @property int|null $item_unit_types_count
 * @property \Illuminate\Database\Eloquent\Collection|\App\Models\Billing\Kardex[] $kardex
 ...
 * @property \Illuminate\Database\Eloquent\Collection|\App\Models\Billing\PurchaseItem[] $purchase_item
 * @property int|null $purchase_item_count
 * @property AffectationIgvType $sale_affectation_igv_type
 * @property \Illuminate\Database\Eloquent\Collection|\App\Models\Billing\SaleNoteItem[] $sale_note_items
 * @property int|null $sale_note_items_count
 * @property \Illuminate\Database\Eloquent\Collection|\App\Models\Billing\ItemSet[] $sets
 * @property int|null $sets_count
 * @property SystemIscType $system_isc_type
 * @property \Illuminate\Database\Eloquent\Collection|\App\Models\Billing\ItemTag[] $tags
 * @property int|null $tags_count
 * @property \Illuminate\Database\Eloquent\Collection|\App\Models\Billing\TechnicalServiceItem[] $technical_service_item
 * @property int|null $technical_service_item_count
 * @property UnitType $unit_type
 * @property \Illuminate\Database\Eloquent\Collection|\App\Models\Billing\ItemWarehousePrice[] $warehousePrices
 * @property int|null $warehouse_prices_count
 * @property \Illuminate\Database\Eloquent\Collection|\App\Models\Billing\ItemWarehouse[] $warehouses
...
class Item extends ModelTenant
{
    ...
    private static function hasWarehouseSupport(): bool
    {
        return class_exists(Warehouse::class) && Schema::connection('tenant')->hasTable('warehouses');
    }

    private static function hasItemWarehousesSupport(): bool
    {
        return class_exists(ItemWarehouse::class) && Schema::connection('tenant')->hasTable('item_warehouses');
    }

    private static function hasItemWarehousePricesSupport(): bool
    {
        return class_exists(ItemWarehousePrice::class) && Schema::connection('tenant')->hasTable('item_warehouse_prices');
    }

    private static function hasTagsSupport(): bool
    {
        return class_exists(ItemTag::class) && Schema::connection('tenant')->hasTable('item_tags');
    }
    ...
    public function scopeWhereWarehouse($query)
     {
        if (!self::hasWarehouseSupport() || !self::hasItemWarehousesSupport()) {
            return $query;
        }
    ...
    public function scopeWhereNotIsSet($query)
    {
        if (!Schema::connection('tenant')->hasTable('items') ||
            !Schema::connection('tenant')->hasColumn('items', 'is_set')) {
            return $query;
        }
    ...
    {
        if (!Schema::connection('tenant')->hasTable('items') ||
            !Schema::connection('tenant')->hasColumn('items', 'active')) {
            return $query;
        }
    ...
    {
        if (!self::hasTagsSupport()) {
            return $this->hasMany(ItemUnitType::class)->whereRaw('1 = 0');
        }
     ...
     *
     * @param \App\Models\Billing\Warehouse|Warehouse $warehouse
    ...
    public function getFullDescription($warehouse, $extended = false) {
        ...
        if ($this->unit_type_id != 'ZZ') {
            if (isset($this['stock'])) {
                ...
                $warehouse_stock = 0;
                if (
                    self::hasWarehouseSupport() &&
                    self::hasItemWarehousesSupport() &&
                    $warehouse &&
                    !empty($warehouse->id)
                ) {
                    $warehouseItems = $this->relationLoaded('warehouses')
                        ? $this->warehouses
                        : $this->warehouses()->get();
                    $itemWarehouse = $warehouseItems->where('warehouse_id', $warehouse->id)->first();
                    $warehouse_stock = $itemWarehouse ? number_format($itemWarehouse->stock, 2) : 0;
                }
            }
            $stock = ($warehouse && !empty($warehouse->id)) ? "{$warehouse_stock}" : '';
        } else {
            $stock = '';
        }
        if($extended == false) {
    ...
    private function getLotsBySerie($warehouse, $series,  $search_item_by_series)
    {
        ...
        if (
            ...
            !empty($warehouse->id) &&
            class_exists(ItemLot::class) &&
            Schema::connection('tenant')->hasTable('item_lots')
        ) {
     ...
     *
     * @param \App\Models\Billing\Warehouse|Warehouse|null $warehouse
 ...
 *
    ...
    public function getDataToItemModal(
        ...
        $realtion_item_unit_types = collect();
        if (Schema::connection('tenant')->hasTable('item_unit_types')) {
            $realtion_item_unit_types = $this->item_unit_types;
        }

        $lots_grp = collect();
        if (class_exists(ItemLotsGroup::class) && Schema::connection('tenant')->hasTable('item_lots_group')) {
            try {
        ...
        if (!empty($warehouse->id) && self::hasItemWarehousesSupport()) {
        ...
        $user = auth()->user();
        $stockPerCategory = [];
        if ($user && class_exists(ItemMovement::class) && method_exists(ItemMovement::class, 'getStockByCategory')) {
            try {
                $stockPerCategory = ItemMovement::getStockByCategory($this->id, $user->establishment_id);
            } catch (Throwable $e) {
                $stockPerCategory = [];
            }
        }
        ...
        $saleUnitPrice = number_format($this->sale_unit_price, 4, ".", "");
        if (!empty($warehouse->id) && self::hasItemWarehousePricesSupport()) {
            $saleUnitPrice = self::getSaleUnitPriceByWarehouse($this, (int) $warehouse->id);
        }

        $warehouses = collect();
        if (self::hasItemWarehousesSupport()) {
        ...
        $data = [
            ...
            'item_unit_types'                  => $realtion_item_unit_types->transform(function ($item_unit_types) {
    ...
    public function getCollectionData(Configuration $configuration = null){
        ...
        $itemWarehousePrices = collect();
        if (self::hasItemWarehousePricesSupport()) {
            try {
                $itemWarehousePrices = $this->warehousePrices->transform(function ($row) {
                    return [
                        'id' => $row->id,
                        'item_id' => $row->item_id,
                        'warehouse_id' => $row->warehouse_id,
        ...
        return [
            ...
            'item_warehouse_prices' => $itemWarehousePrices,
            'is_for_production'=>$this->isIsForProduction(),
            'supplies' => $itemSupply,
