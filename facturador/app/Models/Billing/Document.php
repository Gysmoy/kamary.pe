<?php
    ...
    use App\Http\Controllers\Billing\DownloadController;
    ...
    class Document extends ModelTenant
    {
        ...
        {
            return route('api.download.external_id', ['model' => 'document', 'type' => 'xml', 'external_id' => $this->external_id]);
        }
        ...
        {
            return route('api.download.external_id', ['model' => 'document', 'type' => 'pdf', 'external_id' => $this->external_id]);
        }
        ...
        {
            return route('api.download.external_id', ['model' => 'document', 'type' => 'cdr', 'external_id' => $this->external_id]);
        }
        ...
        public function getNvCollection()
        {
            ...
            $hasDocumentIdColumn = $schema->hasColumn('sale_notes', 'document_id');

            if ($hasDocumentIdColumn) {
                $query->where('document_id', $this->id);

                if (!empty($this->sale_note_id)) {
                    $query->orWhere('id', $this->sale_note_id);
                }
            } elseif (!empty($this->sale_note_id)) {
                $query->where('id', $this->sale_note_id);
            } else {
                return collect();
            }

            return $query->get()->transform(function ($sale_note) {
        ...
        {
            if (!class_exists(OrderNote::class)) {
                return [];
            }
