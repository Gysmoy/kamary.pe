<?php
    ...
    use App\Models\Billing\ItemWarehouse;
    ...
    class SearchItemController extends Controller
    {
        ...
        protected static function hasWarehouseSupport(): bool
        {
            return class_exists(Warehouse::class) && Schema::connection('tenant')->hasTable('warehouses');
        }

        protected static function hasWarehousePricesSupport(): bool
        {
            return class_exists(\App\Models\Billing\ItemWarehousePrice::class)
                && Schema::connection('tenant')->hasTable('item_warehouse_prices');
        }
        ...
        public static function getAllItemBase(Request $request = null, $service = false, $id = 0)
        {
            ...
            try {
                ...
                $search_item_by_series = $configuration ? (bool) $configuration->isSearchItemBySeries() : false;
            } catch (Throwable $e) {
                $search_item_by_series = false;
            }
            ...
            if($production !== false) {
                ...
                if (self::hasWarehousePricesSupport()) {
                    ...
                    if ($search_by_barcode === true) {
                        ...
                        else
                        {
                            if (Schema::connection('tenant')->hasTable('items')
                                && Schema::connection('tenant')->hasColumn('items', 'barcode')) {
                                $item
                                    ->where('barcode', $input)
                                    ->limit(1);
                            } else {
                                $item
                                    ->where('internal_id', $input)
                                    ->limit(1);
                            }
                        }
        ...
        protected static function setFilter(&$item, Request $request = null)
        {
            ...
            if (!empty($input)) {
                $hasBarcodeColumn = Schema::connection('tenant')->hasTable('items')
                    && Schema::connection('tenant')->hasColumn('items', 'barcode');
                $hasAttributesColumn = Schema::connection('tenant')->hasTable('items')
                    && Schema::connection('tenant')->hasColumn('items', 'attributes');
        ...
        ): \Illuminate\Database\Eloquent\Collection
        {
            return $data
                ...
                ->transform(function ($row) use ($warehouse_id) {
                    /** @var \App\Models\Billing\Item $row */
