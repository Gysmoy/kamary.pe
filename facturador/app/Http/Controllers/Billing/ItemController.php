...
use App\Http\Resources\Billing\ItemCollection;
use App\Http\Resources\Billing\ItemResource;
use App\Imports\CatalogImport;
use App\Imports\ItemsImport;
use App\Models\Billing\Catalogs\AffectationIgvType;
use App\Models\Billing\Catalogs\AttributeType;
...
use App\Models\Billing\Catalogs\CatItemUnitsPerPackage;
use App\Models\Billing\Catalogs\ChargeDiscountType;
use App\Models\Billing\Catalogs\CurrencyType;
use App\Models\Billing\Catalogs\OperationType;
use App\Models\Billing\Catalogs\PriceType;
use App\Models\Billing\Catalogs\SystemIscType;
use App\Models\Billing\Catalogs\Tag;
use App\Models\Billing\Catalogs\UnitType;
use App\Models\Billing\CatItemSize;
...
use App\Models\Billing\ItemImage;
use App\Models\Billing\ItemMovement;
use App\Models\Billing\ItemSupply;
use App\Models\Billing\ItemTag;
use App\Models\Billing\ItemUnitType;
use App\Models\Billing\ItemWarehousePrice;
...
class ItemController extends Controller
{
    ...
    public function tables()
    {
        ...
        $warehouses = collect();
        if (class_exists(Warehouse::class) && Schema::connection('tenant')->hasTable('warehouses')) {
            $warehouses = Warehouse::all();
        }
        $accounts = collect();
        if (class_exists(Account::class) && Schema::connection('tenant')->hasTable('accounts')) {
            $accounts = Account::all();
        }

        $tags = collect();
        if (Schema::connection('tenant')->hasTable('tags')) {
            try {
                $tags = Tag::all();
            } catch (Throwable $e) {
                $tags = collect();
        ...
        $categories = collect();
        if (class_exists(Category::class) && Schema::connection('tenant')->hasTable('categories')) {
            try {
        ...
        $brands = collect();
        if (class_exists(Brand::class) && Schema::connection('tenant')->hasTable('brands')) {
            try {
                $brands = Brand::all();
            } catch (Throwable $e) {
                $brands = collect();
        ...
        $CatItemSize= $colors;
        if (method_exists($configuration, 'isShowExtraInfoToItem') && $configuration->isShowExtraInfoToItem()) {
            try {
                ...
                $colors = collect();
                $CatItemStatus = collect();
                $CatItemSize = collect();
                $CatItemUnitBusiness = collect();
                $CatItemMoldCavity = collect();
                $CatItemPackageMeasurement = collect();
                $CatItemUnitsPerPackage = collect();
                $CatItemMoldProperty = collect();
                $CatItemProductFamily = collect();
            }
        }
        /** Informacion adicional */
        $configuration = method_exists($configuration, 'getCollectionData')
            ? $configuration->getCollectionData()
            : [];

        $inventory_configuration = null;
        if (class_exists(InventoryConfiguration::class) && Schema::connection('tenant')->hasTable('inventory_configurations')) {
     ...
     *
     * @return \App\Http\Resources\Billing\ItemCollection
     */
