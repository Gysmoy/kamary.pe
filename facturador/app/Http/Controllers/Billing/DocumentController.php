...
use App\Http\Resources\Billing\DocumentCollection;
use App\Http\Resources\Billing\DocumentResource;
...
use App\Models\Billing\PaymentCondition;
use App\Models\Billing\PaymentMethodType;
...
class DocumentController extends Controller
{
    ...
    public function tables()
    {
        ...
        $business_turns = collect();
        if (class_exists(BusinessTurn::class) && Schema::connection('tenant')->hasTable('business_turns')) {
            try {
        ...
        $payment_conditions = collect();
        if (Schema::connection('tenant')->hasTable('payment_conditions')) {
            try {
...
//        $response = $fact->getResponse();
    ...
    public function consultCdr($document_id)
    {
        try {
            ...
            $response = DB::connection('tenant')->transaction(function () use ($document) {
                $facturalo = new Facturalo();
                $facturalo->setDocument($document);
                $facturalo->consultCdr();
                return $facturalo->getResponse();
            });

            return [
                'success' => true,
                'message' => $response['description'] ?? 'Consulta CDR ejecutada correctamente.',
            ];
        } catch (Throwable $e) {
            return [
                'success' => false,
    ...
    public function sendServer($document_id, $query = false)
    {
        ...
        if (!$document->data_json)
            throw new Exception("Campo data_json nulo o inválido - Comprobante: {$document->fullnumber}");
    ...
    private function getPaymentDestinations(): array
    {
        return [
            [
                'id' => 'cash',
    ...
    public function data_table()
    {
        ...
        return compact('customers', 'document_types', 'series', 'establishments', 'state_types', 'items', 'categories', 'is_client');
    ...
    public function getDataTableCustomers(Request $request)
    {
