...
class Template
{
    ...
    public function validate_template($base_template, $template, $format_pdf)
    {
        ...
        $base_template = !empty($base_template) ? $base_template : 'default';

        $candidates = [
            'pdf'.DIRECTORY_SEPARATOR.$base_template.DIRECTORY_SEPARATOR.$template.'_'.$format_pdf,
            'pdf'.DIRECTORY_SEPARATOR.'default'.DIRECTORY_SEPARATOR.$template.'_'.$format_pdf,
            'pdf'.DIRECTORY_SEPARATOR.'default'.DIRECTORY_SEPARATOR.'invoice_a4',
        ];

        foreach ($candidates as $path_template) {
        ...
        return 'pdf.default.invoice_a4';
    }

    public function validate_preprinted_template($base_template, $template, $format_pdf)
    {
        $path_app_template = app_path('CoreFacturalo'.DIRECTORY_SEPARATOR.'Templates');
        $base_template = !empty($base_template) ? $base_template : 'custom';

        $candidates = [
            'preprinted_pdf'.DIRECTORY_SEPARATOR.$base_template.DIRECTORY_SEPARATOR.$template.'_'.$format_pdf,
            'preprinted_pdf'.DIRECTORY_SEPARATOR.'custom'.DIRECTORY_SEPARATOR.$template.'_'.$format_pdf,
            'preprinted_pdf'.DIRECTORY_SEPARATOR.'static'.DIRECTORY_SEPARATOR.$template.'_'.$format_pdf,
        ];
        ...
        return 'preprinted_pdf.custom.invoice_a4';
    }
