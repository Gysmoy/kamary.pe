...
<VoidedDocuments xmlns="urn:sunat:names:specification:ubl:peru:schema:xsd:VoidedDocuments-1"
    ...
    @php($dispatch = $row->dispatch)
    <sac:VoidedDocumentsLine>
        <cbc:LineID>{{ $loop->iteration }}</cbc:LineID>
        <cbc:DocumentTypeCode>{{ $dispatch->document_type_id ?: '09' }}</cbc:DocumentTypeCode>
        <sac:DocumentSerialID>{{ $dispatch->series }}</sac:DocumentSerialID>
        <sac:DocumentNumberID>{{ $dispatch->number }}</sac:DocumentNumberID>
