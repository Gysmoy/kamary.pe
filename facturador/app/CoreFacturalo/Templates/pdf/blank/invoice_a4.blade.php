@php
    ...
    $hasDispatchesTable = \Illuminate\Support\Facades\Schema::connection('tenant')->hasTable('dispatches');
    $hasDocumentFeeTable = \Illuminate\Support\Facades\Schema::connection('tenant')->hasTable('document_fee');
    $hasQuotationModel = class_exists('App\\Models\\Billing\\Quotation');
...
@if ($hasDispatchesTable && $document->reference_guides)
...
<table class="full-width mt-3">
    ...
    @endif
    @if ($hasQuotationModel && $document->quotation_id)
        <tr>
    ...
    @endif
    @if ($hasQuotationModel && isset($document->quotation->sale_opportunity))
        <tr>
    ...
    @foreach($document->items as $row)
        <tr>
            ...
            <td class="text-center align-top">
                ...
                @php
                    $selectedLotId = isset($row->item->IdLoteSelected) ? $row->item->IdLoteSelected : null;
                @endphp
                @if($selectedLotId)
                    {{ $itemLotGroup->getLote($selectedLotId) }}
                @endif
