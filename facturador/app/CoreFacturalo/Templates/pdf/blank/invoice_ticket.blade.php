@php
    ...
    $hasDispatchesTable = \Illuminate\Support\Facades\Schema::hasTable('dispatches');
    $referenceGuides = collect();
    if ($hasDispatchesTable) {
        $document->load('reference_guides');
        $referenceGuides = $document->reference_guides ?? collect();
    }
    $hasQuotationModule = class_exists(\App\Models\Billing\Quotation::class)
        && \Illuminate\Support\Facades\Schema::hasTable('quotations');
    $quotation = null;
    if ($hasQuotationModule && !empty($document->quotation_id)) {
        $quotation = $document->quotation;
    }
    $fees = collect();
    if (\Illuminate\Support\Facades\Schema::hasTable('document_fee')) {
        $fees = $document->fee;
    }
...
<table class="full-width">
    ...
    @if (!is_null($quotation))
        <tr>
            ...
            <td><p class="desc">{{ $quotation->identifier }}</p></td>
        </tr>
    @endif
    @if (!is_null($quotation) && !empty($quotation->delivery_date))
        <tr>
            <td><p class="desc">T. Entrega</p></td>
            <td><p class="desc">{{ $quotation->delivery_date}}</p></td>
        </tr>
    @endif
</table>
...
@if ($referenceGuides->count() > 0)
<br/>
<strong>Guias de remisión</strong>
<table>
    @foreach($referenceGuides as $guide)
        <tr>
            <td>{{ $guide->series }}</td>
            <td>-</td>
            <td>{{ $guide->number }}</td>
