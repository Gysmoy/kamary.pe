...
class DocumentInput
{
    ...
    public static function set($inputs)
    {
        ...
        // Lite mode: offline module may be removed, so do not force-load OfflineConfiguration.
    ...
    private static function resolveUserId()
    {
        ...
        $user = User::query()->select('id')->orderBy('id')->first();
    ...
    public static function getNameProductXml($row, $inputs)
    {
        ...
        if (in_array($inputs['document_type_id'], ['01', '03'])) {
            ...
            if (!Schema::connection('tenant')->hasTable('configurations') ||
                !Schema::connection('tenant')->hasColumn('configurations', 'name_product_pdf_to_xml')) {
                return null;
            }

            // validar configuracion
            $configuration = Configuration::select('name_product_pdf_to_xml')->first();

            if ($configuration && $configuration->name_product_pdf_to_xml) {
