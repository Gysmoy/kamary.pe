...
class Functions
{
    ...
    public static function person($inputs, $type) {
        ...
        if (Schema::connection('tenant')->hasColumn('persons', 'address_type_id')) {
            $updateData['address_type_id'] = $inputs['address_type_id'];
        }
    ...
    public static function validateDateOfIssue($inputs) 
    {
        ...
        if (
            !Schema::connection('tenant')->hasColumn('configurations', 'shipping_time_days') ||
            !Schema::connection('tenant')->hasColumn('configurations', 'restrict_receipt_date')
        ) {
            return;
        }

        $configuration = Configuration::select('shipping_time_days', 'restrict_receipt_date')->first();
        if (!$configuration || !$configuration->restrict_receipt_date) {
            return;
        }
