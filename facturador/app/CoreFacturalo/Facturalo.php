...
use App\Http\Controllers\Billing\EmailController;
use Exception;
use Mpdf\Mpdf;
...
use App\Models\Billing\Perception;
...
class Facturalo
{
    ...
    public function save($inputs)
    {
        ...
        switch ($this->type) {
            case 'debit':
            case 'credit':
                $document = Document::create($this->filterByExistingColumns('documents', $inputs));
            ...
            case 'invoice':
                ...
                break;
            case 'summary':
                $document = Summary::create($this->filterByExistingColumns('summaries', $inputs));
                ...
                break;
            case 'voided':
                $document = Voided::create($this->filterByExistingColumns('voided', $inputs));
                foreach ($inputs['documents'] as $row) {
                    $document->documents()->create($row);
                }
            ...
            case 'retention':
                ...
                break;
            case 'perception':
                $document = Perception::create($this->filterByExistingColumns('perceptions', $inputs));
                foreach ($inputs['documents'] as $row) {
                    $document->documents()->create($row);
                }
    ...
    public function updateState($state_type_id)
    {
        ...
        if ($schema->hasColumn('documents', 'soap_shipping_response')) {
            $payload['soap_shipping_response'] = isset($this->response['sent']) ? $this->response : null;
        }
    ...
    public function createPdf($document = null, $type = null, $format = null, $output = 'pdf')
    {
        ...
        $template = new Template();
        $mpdfTempDir = storage_path('app' . DIRECTORY_SEPARATOR . 'mpdf-temp');
        if (!is_dir($mpdfTempDir)) {
            @mkdir($mpdfTempDir, 0775, true);
        }
        if (!is_writable($mpdfTempDir)) {
            @chmod($mpdfTempDir, 0775);
        }
        $pdf = new Mpdf([
            'tempDir' => $mpdfTempDir,
        ]);
        ...
        // dd($this->document);
        $base_pdf_template = $this->getBasePdfTemplate();
        if (
            ($format_pdf === 'ticket') OR
            ($format_pdf === 'ticket_58') OR
            ($format_pdf === 'ticket_50')
        ) {
            $base_pdf_template = $this->getBaseTicketPdfTemplate();
        }
        ...
        if (
            ...
            if (config('tenant.enabled_template_ticket_80'))
                $width = 76;
            if (config('tenant.enabled_template_ticket_70'))
                $width = 70;
            if ($format_pdf === 'ticket_50')
                $width = 45;
            ...
            $document_transport = 0;
            if (class_exists(\Modules\BusinessTurn\Models\DocumentTransport::class)) {
                try {
...
//            $pdf->SetHTMLFooter($html_footer);
    ...
    {
        // In lite/demo mode we don't depend on SUNAT parsing to complete the flow.
        if ($this->isDemo) {
            $this->response = [
                'sent' => true,
                'code' => 0,
                'description' => 'Documento procesado en modo DEMO',
                'notes' => []
            ];
    ...
    public function updateRegularizeShipping($code, $description)
    {
        ...
        if ($schema->hasColumn('documents', 'regularize_shipping')) {
            $payload['regularize_shipping'] = true;
        }

        if ($schema->hasColumn('documents', 'response_regularize_shipping')) {
            $payload['response_regularize_shipping'] = [
                'code' => $code,
    ...
    {
        if ($this->isDemo) {
            throw new Exception('Consultar CDR no esta disponible en modo DEMO. Usa PRODUCCION para consultar SUNAT.');
        }

        // CDR status must use SUNAT billConsultService, not billService.
        $consultClient = new WsClient('consultCdrStatus');
        $consultClient->setCredentials($this->soapUsername, $this->soapPassword);
        $consultClient->setService(SunatEndpoints::FE_CONSULTA_CDR);

        $consultCdrService = new ConsultCdrService();
        $consultCdrService->setClient($consultClient);
...
//        $this->soapPassword = ($this->isDemo)?'moddatos':$this->company->soap_password;
    ...
    {
        $default_payment_date = $document->date_of_issue ? $document->date_of_issue->format('Y-m-d') : date('Y-m-d');
        $payments = collect($payments)->map(function ($row) use ($default_payment_date) {
            if (!isset($row['date_of_payment']) || empty($row['date_of_payment'])) {
                $row['date_of_payment'] = $default_payment_date;
            }
            return $row;
        })->values()->all();
        ...
        foreach ($payments as $row) {
            ...
            if (isset($row['payment_destination_id']) && method_exists($this, 'createGlobalPayment')) {
    ...
    public function update($inputs, $id)
    {
        ...
        switch ($this->type) {
            case 'invoice':
                ...
                $document->fill($this->filterByExistingColumns('documents', $inputs));
                $document->save();
                ...
                //     /** @var \App\Models\Billing\DocumentItem $item */
    ...
    private function filterByExistingColumns(string $table, array $inputs): array
    {
        try {
            ...
            return array_intersect_key($inputs, array_flip($columns));
        } catch (Exception $e) {
            return $inputs;
        }
    }
    ...
    private function getBasePdfTemplate(): string
    {
        try {
            $defaultTemplate = $this->getGlobalPdfTemplate();

            // En modo lite se prioriza un template global unico para evitar
            // diferencias visuales entre comprobantes.
            if (config('tenant.force_global_pdf_template', true)) {
                return $defaultTemplate;
            }

            $establishment = Establishment::find($this->document->establishment_id);
            $template = $establishment->template_pdf ?? null;

            return $this->isValidPdfTemplate($template) ? $template : $defaultTemplate;
        } catch (Exception $e) {
            return $this->getGlobalPdfTemplate();
        }
    }

    private function getBaseTicketPdfTemplate(): string
    {
        try {
            $defaultTemplate = $this->getGlobalPdfTemplate();
            ...
            $establishment = Establishment::find($this->document->establishment_id);
            $template = $establishment->template_ticket_pdf ?? null;
    ...
    private function getGlobalPdfTemplate(): string
    {
        $template = env('PDF_TEMPLATE', config('tenant.pdf_template', 'blank'));
        return !empty($template) ? $template : 'blank';
    }

    private function isValidPdfTemplate($template): bool
    {
        ...
        $basePath = app_path(
            'CoreFacturalo' . DIRECTORY_SEPARATOR .
