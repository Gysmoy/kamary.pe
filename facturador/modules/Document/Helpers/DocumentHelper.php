...
class DocumentHelper
{
    ...
    {
        try {
            if (!Schema::connection('system')->hasTable('hostnames') || !Schema::connection('system')->hasTable('clients')) {
                return null;
            }

            $host = app()->bound('request')
                ? (string) app('request')->getHost()
            ...
            $hostname = Hostname::query()->where('fqdn', $host)->first();

            if (!$hostname) {
                $hostname = Hostname::query()->first();
            }

            if (!$hostname || !$hostname->id) {
                return null;
            }

            $client = Client::select('start_billing_cycle')->where('hostname_id', $hostname->id)->first();

            return optional($client)->start_billing_cycle;
        } catch (Throwable $e) {
            return null;
        }
    }
