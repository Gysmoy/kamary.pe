...
import SalesRest from './Actions/Customer/sales-rest';
...
const SaleDetailModal = ({ sale, onClose, onReceivedClicked, onReportClicked }) => {
  useEffect(() => {
      ...
      <div className="bg-white rounded-xl shadow-lg w-full max-w-2xl mx-4 max-h-[90vh] overflow-y-auto p-6 space-y-6 relative" onClick={e => e.stopPropagation()}>
          ...
          <div className='flex flex-wrap gap-2 justify-end'>
            <div className="p-2 border bg-gray-50 text-xs rounded-full w-max text-center">
              {sale.status.name}
                ...
                {/* Show reject reason if status is rejected or issue */}
        {(sale.status_id === 'f47ac50f-58cc-11ef-8f8e-0242ac120002' || sale.status_id === 'f47ac610-58cc-11ef-8f8e-0242ac120002') && (
          <div className="mt-4 p-4 bg-red-50 border border-red-200 rounded-lg">
            <h4 className="font-semibold text-red-800 mb-2">Motivo de rechazo</h4>
            <p className="text-red-700 text-sm">{sale.reject_reason}</p>
          </div>
        )}
        <div className='border rounded-xl p-6'>
          ...
          <div className='mt-4 text-sm'>
            <span className='text-silver'>Lo entrega:</span> {sale.seller.fullname}
          </div>
          {
            ...
            sale.ready_for_pickup_at && <>
              ...
              <div className="mt-2 border-s-2 text-xs text-silver">
                <i className="mdi mdi-information-outline" />
                <span>Tienes 72 horas para recogerlo o se marcará como entregado</span>
              </div>
              <div className="flex flex-wrap gap-2 mt-4">
                <button
                  onClick={() => onReceivedClicked(sale.id)}
                  className="px-4 py-2 text-sm bg-primary text-white rounded-lg hover:bg-primary-dark transition-colors"
                >
                  Marcar como recibido
                </button>
                <button
                  onClick={() => onReportClicked(sale.id)}
...
const Orders = ({ statuses }) => {
  ...
  const onReceivedClicked = async (id) => {
    const { isConfirmed } = await Swal.fire({
      title: '¿Marcar como entregado?',
      text: 'Esta acción es irreversible. Asegúrese de que ha recibido el pedido.',
      icon: 'warning',
    ...
    const result = await salesRest.save({ id, status_id: 'f47ac40e-58cc-11ef-8f8e-0242ac120002' });
    if (!result) return
  ...
  const onReportClicked = async (id) => {
    const { value: reject_reason } = await Swal.fire({
      title: '¿Reportar pedido?',
      text: 'Esta acción es irreversible. Por favor, indique el motivo del reporte.',
      input: 'textarea',
      inputPlaceholder: 'Escriba el motivo del reporte...',
      icon: 'warning',
      showCancelButton: true,
      ...
      inputValidator: (value) => {
        if (!value) {
          return 'Debe ingresar un motivo para reportar';
        }
      }
    });
    ...
    const result = await salesRest.save({ id, status_id: 'f47ac610-58cc-11ef-8f8e-0242ac120002', reject_reason });
    if (!result) return
  ...
  return <>
    <section className="w-full bg-[#EFF3F5] py-12 sm:py-16">
      <div className={`max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6`}>
        <div>
          <h4 className='text-lg font-bold mb-1'>Mi compras</h4>
              ...
              {loadingSales ? (
                ...
                sales.map((sale, index) => {
                  ...
                  return <tr className={index > 0 && 'border-t'} key={sale.id}>
                    <td className='py-6 px-4 text-primary font-medium'>#MS{String(sale.id).padStart(8, '0')}</td>
    ...
    {selectedSale && <SaleDetailModal sale={selectedSale} onClose={closeSaleDetail} onReceivedClicked={onReceivedClicked} onReportClicked={onReportClicked} onMarkReadyClicked={onMarkReadyClicked} />}
  </>;
};

CreateReactScript((el, properties) => {
  createRoot(el).render(
    <Base {...properties} title='Mis compras'>
      <Orders {...properties} />
    </Base>
  );
});
