...
class KardexRest extends BasicRest {
  ...
  saveWarehouse = async (request) => {
    try {
      const { status, result } = await Fetch('/api/admin/warehouses', {
        method: 'POST',
  ...
  deleteWarehouse = async (id) => {
    try {
      const { status, result } = await Fetch(`/api/admin/warehouses/${id}`, {
        method: 'DELETE'
      })
  ...
  saveLocation = async (request) => {
    try {
      const { status, result } = await Fetch('/api/admin/kardex/locations', {
        method: 'POST',
        body: JSON.stringify(request)
      })
  ...
  deleteLocation = async (id) => {
    try {
      const { status, result } = await Fetch(`/api/admin/kardex/locations/${id}`, {
        method: 'DELETE'
      })
