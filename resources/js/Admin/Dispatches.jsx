    ...
    <Table
      ...
      columns={[
        ...
        { caption: 'Acciones', width: 315, allowFiltering: false, allowExporting: false, cellTemplate: (container, { data }) => {
          ...
          container.append(DxButton({ className: 'btn btn-xs btn-soft-warning ms-1', title: dispatchGuides(data).length ? 'Ver guias' : 'Generar guias', icon: 'mdi mdi-file-document-plus-outline', onClick: () => onShowGuides(data) }))
          container.append(DxButton({ className: 'btn btn-xs btn-soft-success ms-1', title: 'Ver evidencias', icon: 'mdi mdi-image-check-outline', onClick: () => onShowEvidences(data) }))
